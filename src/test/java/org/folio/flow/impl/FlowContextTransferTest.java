...
public class FlowContextTransferTest {
  ...
  void execute_positive_contextData(FlowEngine flowEngine) {
    ...
    doAnswer(inv -> setParameterInContext(inv, "p", "v3")).when(stage3).execute(any());
    doAnswer(inv -> setParameterInContext(inv, "c", "w3")).when(stage3).cancel(any());
    ...
    verify(stage1).cancel(StageContext.of(flowId, flowParams, Map.of("p", "v3", "c", "w1")));

    verify(stage2).execute(StageContext.of(flowId, flowParams, Map.of("p", "v2")));
    verify(stage2).cancel(StageContext.of(flowId, flowParams, Map.of("p", "v3", "c", "w2")));

    verify(stage3).execute(StageContext.of(flowId, flowParams, Map.of("p", "v3")));
    verify(stage3).cancel(StageContext.of(flowId, flowParams, Map.of("p", "v3", "c", "w3")));

    verify(stage4).execute(StageContext.of(flowId, flowParams, Map.of("p", "v3")));
    verify(stage4).recover(StageContext.of(flowId, flowParams, Map.of("p", "v3")));
  }
